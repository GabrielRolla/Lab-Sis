/*
 * ============================================================================
 * GRAVADOR DE ÁUDIO NO SD COM FILTRAGEM DSP E DETECÇÃO VAD (ZCR + Energia)
...
// ----- CONFIGURAÇÕES DO DETECTOR AUTÔNOMO DE VOZ (VAD) -----
// Limite mínimo de volume para o ESP32 considerar que não é silêncio/ruído de fundo. 
// Como o áudio varia de -32768 a 32767, 300 é um valor de energia baixo, mas suficiente para ignorar estática.
const float ENERGY_THRESHOLD = 300.0f; 

// Limites de Cruzamento por Zero (ZCR) para um buffer de 512 amostras a 16kHz
// 300Hz geram ~19 cruzamentos por buffer. 3000Hz geram ~192 cruzamentos.
const int ZCR_MIN = 15;  
const int ZCR_MAX = 200; 

// Memória para o cálculo do ZCR (precisamos saber se a amostra anterior era positiva ou negativa)
...
void setup() {
  ...
  setupI2S();
  Serial.println("\n>>> GRAVANDO E ANALISANDO VOZ... Fale agora! <<<");
  startTime = millis();
}
...
void loop() {
  if (totalDataSize >= EXPECTED_DATA_SIZE) {
    audioFile.close();
    Serial.println("=====================================");
    Serial.println("GRAVACAO E ANALISE CONCLUIDAS!");
    Serial.println("=====================================");
  ...
  // Variáveis temporárias para a análise deste bloco específico de áudio
  float bufferEnergy = 0.0f;
  int zcrCount = 0;

  // =========================================================================
  // PIPELINE DE PROCESSAMENTO (Filtro + Análise de Voz)
  // =========================================================================
  for (int i = 0; i < samplesRead; i++) {
    ...
    // 4. LÓGICA DO DETECTOR DE VOZ (VAD)
    // Soma a energia (módulo da amplitude) da onda já filtrada
    bufferEnergy += abs(y_voz_lp2);

    // Verifica se a onda cruzou o eixo zero (passou de positivo para negativo ou vice-versa)
    if ((y_voz_lp2 >= 0 && last_filtered_sample < 0) || (y_voz_lp2 < 0 && last_filtered_sample >= 0)) {
        zcrCount++;
    }
    last_filtered_sample = y_voz_lp2; // Guarda o estado para a próxima iteração do loop

    // 5. Armazenamento para o SD Card
    writeBuffer[i] = (int16_t)y_voz_lp2;
  }
  // =========================================================================

  // Tira a média da energia do buffer
  bufferEnergy = bufferEnergy / samplesRead;

  // Se o som for alto o suficiente E a taxa de cruzamento estiver na janela da voz humana...
  if (bufferEnergy > ENERGY_THRESHOLD && zcrCount > ZCR_MIN && zcrCount < ZCR_MAX) {
      Serial.println(">>> ALERTA: POSSIVEL VOZ DETECTADA! <<<");
  }

  // Grava o buffer no SD
